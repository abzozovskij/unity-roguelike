...
public class BossEnemy : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (playerInAttackRange && !stompOnCooldown && !attacked && canStomp)
        {
            Stomp();
            return;
        }
    ...
    void Stomp()
    {
        ...
        Invoke(nameof(StompDamage), 0.6f);


        Invoke(nameof(ResetStompCooldown), stompCooldown);
    }

    void StompDamage()
    {
        Collider[] hits = Physics.OverlapSphere(transform.position, stompRange);
        GameObject fx = Instantiate(stompEffect, transform.position, Quaternion.identity);
        Destroy(fx, 3f);
        foreach (Collider col in hits)
        {
            ...
            if (p != null)
            {
                p.TakeDamage(stompDamage, false);

                Rigidbody rb = p.GetComponent<Rigidbody>();
                if (rb != null)
                {
                    Vector3 dir = (p.transform.position - transform.position).normalized;
                    rb.AddForce(dir * stompForce, ForceMode.Impulse);
                }
            }
        }
    }
